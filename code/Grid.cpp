    ...
    if (p == nullptr) {
        ...
        if (!vertical) {
            while (this->move_queue.size() > 0) {
				...
				this->move_queue.pop_back();// erase(this->move_queue.end());
            }
            while (this->rot_queue.size() > 0) {
                ...
                p->RotateClockwise();
				this->rot_queue.pop_back();// erase(this->rot_queue.end());
            }
 ...
 */
bool den::Grid::CheckCollision(Uint32 i, Uint32 j) {
 ...
 */
void den::Grid::NewBlock(Uint32 type, Uint32 i, Uint32 j) {
 ...
 */
void den::Grid::SetBlock(Uint32 i, Uint32 j, den::Block *val) {
    if (!this->CheckCollision(i, j)) {
        this->grid[i][j] = val;
    }
    else {
